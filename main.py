...
def main():
    ...
    while gen_counter < n_iter:
        ...
        raw_target_list = []
        volume_list = []
        summed_wav, summed_label, raw_target_list, volume_list = generate_one_sample(rng=rng, max_n_to_mix = syn_params.max_n_to_mix, n_samples = syn_params.n_samples, small_variance = syn_params.small_variance, duration = syn_params.duration, frames_per_sec = syn_params.frames_per_sec, n_frames = syn_params.n_frames,  default_kernel = default_kernel, n_frequencies = syn_params.n_frequencies, noise_anchors = syn_params.noise_anchors, noise_vagueness = syn_params.noise_vagueness, sample_rate = syn_params.sample_rate, workid = args.workid)
        ...
        sf.write(f'{savedir}/' + f'wav/{gen_counter}_{n_to_mix}mix.wav', summed_wav, syn_params.sample_rate, 'PCM_24')
